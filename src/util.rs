...
pub fn readlinkat(path: &fs::File, filename: impl AsRef<Path>) -> Result<OsString> {
    nix::fcntl::readlinkat(path.as_raw_fd(), filename.as_ref())
        .with_context(|| format!("Failed to readlink {}", filename.as_ref().display()))
}
