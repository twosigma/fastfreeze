...
impl Run {
    fn run_inner(self) -> Result<()> {
        ...
        match (app_name, no_container, nscaps.can_mount_ns(), install::is_ff_installed()?) {
            ...
            (_,          _,     false, false) => bail!("`fastfreeze install` must first be executed \
                                                        as mount namespaces are not available"),
            (Some(name), false, true,      _) => container::create(&name, nscaps)?,
            (None,       false, true,      _) => container::create(default_app_name(), nscaps)?,
        ...
        if !detach {
            ...
            // Cleanup the container. Useful if we don't have a container
            // monitor to cleanup after us, which is the case when we have a
            // mount namespace, but no PID namespace.
            container::cleanup_current_container();
        }
