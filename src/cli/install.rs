...
impl super::CLI for Install {
    fn run(self) -> Result<()> {
        ...
        match (container::can_create()?, force) {
            (true, false) => bail!(
                "Installation does not seem necessary \
                because fastfreeze can most likely create containers. \
                Use `--force` if you want to proceed"),
