...
use nix::{
    ...
    unistd::{ForkResult, Pid, dup2, fork, getgid, getuid, close, getpid}
};
use crate::{
    consts::*,
    cli::{ExitCode, install, run::is_app_running},
    process::{monitor_child, ChildDied},
    util::{create_dir_all, set_tmp_like_permissions, openat, readlinkat, setns},
    logger,
};
...
fn prepare_user_namespace() -> Result<()> {
    ...
    // user namespace. This gives us CAP_SYS_ADMIN in our pid namespace, useful
    // to write to /proc/sys/kernel/ns_last_pid
...
impl NSCapabilities {
    ...
    pub fn can_pid_ns(&self) -> bool { *self == Self::Full }
}
...
pub fn ns_capabilities() -> Result<NSCapabilities> {
    fn test_in_child_process(child_fn: impl FnOnce() -> Result<()>) -> Result<bool> {
        match fork()? {
            ForkResult::Child => {
                std::process::exit(if child_fn().is_ok() { 0 } else { 1 });
            }
    ...
    let cap = match (can_create_mount_ns, can_create_pid_ns) {
        ...
        // a mount ns because it's unlikely.
    };

    if log_enabled!(log::Level::Debug) {
        match cap {
            NSCapabilities::None => {
                // Docker blocks the unshare() system call by default with seccomp.
                // That's the end of the road for containers.
                debug!("Running with no container support as we fail to create user and mount namespaces");
            }
            NSCapabilities::MountOnly => {
                // We could be on Kubernetes, where we have a shadow /proc prevending us to
                // use a PID namespace, but we can do mount namespaces.
                info!("PID namespaces are not supported{}",
                    if is_proc_ro_protected()? { " because /proc is read-only protected" } else { "" });
                info!("Running multiple applications is not supported, and controlling PIDs may be slow");
            }
            NSCapabilities::Full => {
                debug!("Running with full container support");
            }
        }
    }
...
pub fn create(name: &str, nscap: NSCapabilities) -> Result<()> {
    ...
    if nscap.can_pid_ns() {
        ...
        // We can't create a PID namespace, but we still want to write down a pid
        // to later allow a "fastfreeze checkpoint" command to use nsenter() for
        // entering the mount namespace.
        write_container_pid_file(getpid())?;
    }
...
pub fn nsenter(name: &str) -> Result<()> {
    ...
    // Next we setns() all the namespaces that are different from ours.
    // This is a useful thing to do when we don't have a pid namespace,
    // but have a mount namespace.
    let self_proc_dir = fs::File::open("/proc/self")
        .with_context(|| format!("Failed to open {}", "/proc/self"))?;
    ...
    let mut applied_setns = CloneFlags::empty();

    for (ns_path, clone_flag) in &namespaces {
        let current_ns = readlinkat(&self_proc_dir, ns_path)?;
        let target_ns = readlinkat(&container_proc_dir, ns_path)?;

        if current_ns != target_ns {
            let nsfile = openat(&container_proc_dir, ns_path)?;
            setns(&nsfile, *clone_flag)?;
            applied_setns |= *clone_flag;
        }
    }

    // If we setns a pid namespace, we fork to get into it.
    if applied_setns.contains(CloneFlags::CLONE_NEWPID) {
...
pub fn cleanup_current_container() {
