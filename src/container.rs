    ...
    if log_enabled!(log::Level::Debug) {
        match cap {
            ...
            NSCapabilities::MountOnly => {
                ...
                let extra_msg = if is_proc_ro_protected()? {
                    " because /proc has subdirectories that are read-only, preventing us from remounting /proc"
                } else { "" };
                debug!("Running with partial container support. \
                        We can create user and mount namespaces, but not PID namespaces{}", extra_msg);
            }
            NSCapabilities::Full => {
