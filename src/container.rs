...
use nix::{
    ...
    unistd::{ForkResult, Pid, dup2, fork, close, getuid, getgid}
};
...
fn prepare_user_namespace() -> Result<()> {
    ...
    // We set CAP_SYS_ADMIN as an ambient capability to allow child process such
    // as criu or set_ns_last_pid to write to /proc/sys/kernel/ns_last_pid
    caps::raise(None, CapSet::Inheritable, Capability::CAP_SYS_ADMIN)
        .context("Failed to make CAP_SYS_ADMIN inheritable")?;

    caps::raise(None, CapSet::Ambient, Capability::CAP_SYS_ADMIN)
        .context("Failed to make CAP_SYS_ADMIN ambient")?;

    // We preserve our uid/gid to make things as transparent as possible for the user.
    // No need to get uid=0 in our namespace as we've set CAP_SYS_ADMIN as ambient.
